...
action :install do
  ...
  unless features_to_install.empty?
    ...
    converge_by(message) do
      ...
      shell_out!("#{dism} /online /enable-feature #{features_to_install.map { |f| "/featurename:#{f}" }.join(' ')} /norestart #{addsource} #{addall}", returns: [0, 42, 127, 3010], timeout: new_resource.timeout)
...
action_class do
  ...
  def fail_if_removed
    ...
    if node['os_version'].to_f > 6.2
      return if registry_value_exists?('HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\Servicing', name: 'LocalSourcePath') # if source is defined in the registry, still fine
    end
