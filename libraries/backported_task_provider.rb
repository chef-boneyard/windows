...
class Chef
  class Provider
    class WindowsTask < Chef::Provider
      ...
      provides :windows_task, os: "windows" do
        Gem::Requirement.new(">= 13.0", "< 13.7").satisfied_by?(Gem::Version.new(Chef::VERSION))
      end
      ...
      def action_create
        Chef::Log.info("Running the windows_task resource backported from Chef 13.7 located in the Windows cookbook. Upgrading to Chef 13.7+ will use the build in resource instead.")
